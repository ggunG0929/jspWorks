...
public class ProductDAO {
	...
	// 상품 수정(이미지 수정시 이미지를 바꿈)
	public void updateProduct(Product product) {
		conn = JDBCUtil.getConnection();
		String sql = "UPDATE product SET p_name=?, p_unitPrice=?, p_description=?, "
				+ "p_category=?, p_manufacturer=?, p_unitsInStock=?, p_condition=?, p_productImage=? WHERE p_id=?";
		try {
			pstmt = conn.prepareStatement(sql);		// surround with try catch
	...
	// 상품 수정2(이미지 수정이 없으면 이미지를 유지)
	public void updateProductNoImage(Product product) {
		conn = JDBCUtil.getConnection();
		String sql = "UPDATE product SET p_name=?, p_unitPrice=?, p_description=?, "
				+ "p_category=?, p_manufacturer=?, p_unitsInStock=?, p_condition=? WHERE p_id=?";
		try {
			pstmt = conn.prepareStatement(sql);		// surround with try catch
			pstmt.setString(1, product.getPname());
			pstmt.setInt(2, product.getUnitPrice());
