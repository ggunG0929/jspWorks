create table t_board(
	bnum	int primary key auto_increment,		-- 게시글 번호	-- 자동순번
    title	varchar(100) not null,				-- 글 제목
    content 	text,							-- 글 내용
    regdate 	timestamp default now(),		-- 작성일
    modifydate	timestamp,						-- 수정일
    hit		int default 0,						-- 조회수
    memberid	varchar(20),					-- 작성자
    fileupload	varchar(50),					-- 첨부파일
    constraint fk_member_board foreign key(memberid)	-- 외래키 제약조건
    references t_member(memberid) on delete cascade
);

-- 댓글 테이블
create table t_reply(
	rno			int primary key auto_increment,		-- 댓글번호 자동순번
